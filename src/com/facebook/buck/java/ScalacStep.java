 ...
 * will invoke scalac using {@code declaredClasspathEntries} for the classpath.
 * If {@code buildDependencies} is set to {@link BuildDependencies#TRANSITIVE}, this class will
 * invoke scalac using {@code transitiveClasspathEntries} for the classpath.
...
public class ScalacStep implements Step {
  ...
  /**
   * Will be {@code true} once {@link Scalac#buildWithClasspath(ExecutionContext, BuildTarget,
  ...
  public int executeBuild(ExecutionContext context) throws IOException, InterruptedException {
    ...
    if (buildDependencies == BuildDependencies.FIRST_ORDER_ONLY) {
      return getScalac().buildWithClasspath(
          context,
          invokingRule,
  ...
  private int tryBuildWithFirstOrderDeps(ExecutionContext context)
    ...
    try (ExecutionContext firstOrderContext = context.createSubContext(stdout, stderr)) {
      ...
      if (declaredDepsResult != 0) {
        int transitiveResult = scalac.buildWithClasspath(
            context,
            invokingRule,
            getOptions(context, transitiveClasspathEntries),
            scalaSourceFilePaths,
  ...
  @VisibleForTesting
  Scalac getScalac() {
    return scalacOptions.getScalac(resolver);
  }
