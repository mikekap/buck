 ...
 * )
 * </pre>
 * Then this would compile {@code FeedStoryRenderer.scala} against Guava and the classes generated
 * from the {@code //src/com/facebook/feed/model:model} rule.
 */
public class DefaultScalaLibrary extends AbstractBuildRule
  ...
  private final BuildRule scalaLibrary;
  ...
  private ScalacOptions scalacOptions;
   ...
   * @param scalacOptions scalac configuration.
  ...
  void createCommandsForScalac(
    ...
    // Only run scalac if there are .scala files to compile.
    if (!getScalaSrcs().isEmpty()) {
      ...
      ScalacStep scalacStep = new ScalacStep(
          outputDirectory,
          workingDirectory,
          getScalaSrcs(),
  ...
  private static Path getClassesDir(BuildTarget target) {
    return BuildTargets.getGenPath(target, "lib__%s__classes");
  }
  ...
  public RuleKey.Builder appendDetailsToRuleKey(RuleKey.Builder builder) {
    ...
    return scalacOptions.appendToRuleKey(builder, "scalacOptions");
  }
  ...
  public ImmutableSortedSet<Path> getJavaSrcs() {
    return ImmutableSortedSet.<Path>of();
  }

  @Override
  public ImmutableSortedSet<BuildRule> getDepsForTransitiveClasspathEntries() {
    Set<BuildRule> deps = Sets.union(getDeclaredDeps(), exportedDeps);
    deps = Sets.union(deps, ImmutableSet.of(scalaLibrary));
    return ImmutableSortedSet.copyOf(deps);
  }
  ...
  public ScalacOptions getScalacOptions() {
    return scalacOptions;
  }

  /**
   * Building a scala_library() rule entails compiling the .scala files specified in the srcs
  ...
  public final ImmutableList<Step> getBuildSteps(
    ...
    if (getProperties().is(ANDROID)) {
      this.scalacOptions = ScalacOptions.builder(scalacOptions)
    ...
    // Always create the output directory, even if there are no .scala files to compile because there
    // might be resources that need to be copied there.
    BuildTarget target = getBuildTarget();
    Path outputDirectory = getClassesDir(target);
    steps.add(new MakeCleanDirectoryStep(outputDirectory));

    Optional<ScalacStep.SuggestBuildRules> suggestBuildRule =
    ...
    if (outputJar.isPresent()) {
      ...
      Path scratch = BuildTargets.getGenPath(
          target,
  ...
  /**
   * @return A function that takes a list of failed imports from a scalac invocation and returns a
   *    set of rules to suggest that the developer import to satisfy those imports.
   */
  @VisibleForTesting
  Optional<ScalacStep.SuggestBuildRules> createSuggestBuildFunction(
      BuildContext context,
      ImmutableSetMultimap<JavaLibrary, Path> transitiveClasspathEntries,
      ImmutableSetMultimap<JavaLibrary, Path> declaredClasspathEntries,
    ...
    ScalacStep.SuggestBuildRules suggestBuildRuleFn =
        new ScalacStep.SuggestBuildRules() {
      @Override
   ...
   * outputDirectory of scalac outputs.
   *
   ...
   * is buck-out/bin/scala/abc/lib__abc__classes/, then a contained class abc.AbcModule
   * should be at buck-out/bin/scala/abc/lib__abc__classes/abc/AbcModule.class
   *
   ...
   * @param outputDirectory the directory that will contain all the scalac output.
   */
  @VisibleForTesting
  ...
  public Scalac getScalac() {
    return scalacOptions.getScalac(getResolver());
  }
