...
public class ScalaTest extends DefaultScalaLibrary implements TestRule {
  ...
  private static final Logger LOG = Logger.get(ScalaTest.class);
   ...
   * other {@code scala_test()} rules, then they will be run separately.
   */
  @Override
  public ImmutableList<Step> runTests(
    ...
    // If no classes were generated, then this is probably a scala_test() that declares a number of
    // other scala_test() rules as deps, functioning as a test suite. In this case, simply return an
  ...
  public Path getPathToTestOutputDirectory() {
    ...
    pathsList.add(
        String.format("__scala_test_%s_output__", getBuildTarget().getShortNameAndFlavorPostfix()));
  ...
  private Path getPathToTmpDirectory() {
    Path base = BuildTargets.getGenPath(getBuildTarget(), "__scala_test_%s_tmp__").toAbsolutePath();
    String subdir = BuckConstant.oneTimeTestSubdirectory;
    if (subdir != null && !subdir.isEmpty()) {
  ...
  static class CompiledClassFileFinder {
    ...
    CompiledClassFileFinder(ScalaTest rule, ExecutionContext context) {
      Path outputPath;
    ...
    /**
     * When a collection of .scala files is compiled into a directory, that directory will have a
     * subfolder structure that matches the package structure of the input .scala files. In general,
     * the .scala files will be 1:1 with the .class files with two notable exceptions:
     ...
     * (2) A .scala file that defines multiple top-level classes (yes, this can exist:
    ...
    static Set<String>  getClassNamesForSources(
      ...
      for (Path path : sources) {
        ...
        source = source.substring(0, source.length() - ".scala".length());
